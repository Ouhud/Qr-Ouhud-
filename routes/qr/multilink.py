# routes/qr/multilink.py
# =============================================================================
# 🚀 Multilink QR-Code Routes (Ouhud QR)
# =============================================================================
...
router = APIRouter(prefix="/qr/multilink", tags=["Multilink QR"])
...
def show_form(request: Request) -> HTMLResponse:
    """Zeigt das Multilink QR-Formular."""
    return templates.TemplateResponse("qr_multilink.html", {"request": request})


@router.post("/generate", response_class=HTMLResponse)
async def create_multilink_qr(
    request: Request,
    public_url: str = Form(""),
    public_html: str = Form(""),
    title: str = Form(""),
    style: str = Form("modern"),
    logo: Optional[UploadFile] = File(None),
    db: Session = Depends(get_db),
) -> HTMLResponse:
    """Erstellt einen Multilink QR-Code."""
    
    user_id = request.session.get("user_id")
    if not user_id:
        raise HTTPException(status_code=401, detail="Nicht eingeloggt")
    slug = uuid.uuid4().hex[:10]
    logo_fs_path, logo_public_path = save_qr_logo(logo, slug, "multilink_logo")
    ...
    # Bild speichern
    qr_file = QR_DIR / f"multilink_{slug}.png"
