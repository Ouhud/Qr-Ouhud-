...
router = APIRouter(prefix="/qr/review", tags=["Review QR"])
...
def show_form(request: Request) -> HTMLResponse:
    return templates.TemplateResponse("qr_review_form.html", {"request": request})


@router.post("/generate", response_class=HTMLResponse)
async def generate_review_qr(
    request: Request,
    ...
    logo_fs_path, logo_public_path = save_qr_logo(logo, slug, "review_logo")
    ...
    qr_file = QR_DIR / f"review_{slug}.png"
    ...
    qr = QRCode(
        ...
        style=style,
        title=title or f"Review: {platform}",
    )
    qr.set_data({"platform": platform, "review_url": review_url, "title": title, "logo_path": logo_public_path})
    db.add(qr)
    db.commit()
    db.refresh(qr)
