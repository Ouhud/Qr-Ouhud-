...
router = APIRouter(prefix="/qr/booking", tags=["Booking QR"])
...
def show_form(request: Request) -> HTMLResponse:
    return templates.TemplateResponse("qr_booking_form.html", {"request": request})


@router.post("/generate", response_class=HTMLResponse)
async def generate_booking_qr(
    request: Request,
    provider: str = Form("calendly"),
    ...
    logo_fs_path, logo_public_path = save_qr_logo(logo, slug, "booking_logo")
    ...
    qr_file = QR_DIR / f"booking_{slug}.png"
    ...
    qr = QRCode(
        ...
        style=style,
        title=title or f"Booking: {provider}",
    )
    qr.set_data({"provider": provider, "booking_url": booking_url, "title": title, "logo_path": logo_public_path})
    db.add(qr)
    db.commit()
    db.refresh(qr)
